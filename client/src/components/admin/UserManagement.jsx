...
import { Search, Edit2, Trash2, X, Check, ShieldAlert, Award, User, RefreshCw, ChevronLeft, ChevronRight } from 'lucide-react';
...
const UserManagement = () => {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this user? This action is irreversible and deletes all associated profile and progress data.')) {
      return;
    }
    try {
  ...
  const handleEditOpen = (user) => {
    ...
    setEditVerified(user.isVerified || false);
    setEditLocation(user.profile?.location || '');
    setEditBio(user.profile?.bio || '');
    setEditPhone(user.profile?.phoneNumber || '');
  };
                ...
                users.map((u) => (
                  <tr key={u._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-850/20 transition-colors">
                    <td className="px-6 py-4">
                      <div>
                        <div className="font-bold text-slate-900 dark:text-white font-outfit">{u.name}</div>
                        <div className="text-xs text-slate-400">{u.email}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      {u.role === 'admin' ? (
                        <span className="flex items-center space-x-1 w-fit text-[10px] font-bold px-2 py-0.5 rounded-full bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-450 border border-rose-200 dark:border-rose-900/50">
                          <ShieldAlert className="w-3 h-3" />
                          <span>Admin</span>
                        </span>
                      ) : u.role === 'mentor' ? (
                        <span className="flex items-center space-x-1 w-fit text-[10px] font-bold px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-900/50">
                          <Award className="w-3 h-3" />
                          <span>Mentor</span>
                        </span>
                      ) : (
                        <span className="flex items-center space-x-1 w-fit text-[10px] font-bold px-2 py-0.5 rounded-full bg-primary-50 dark:bg-primary-950/40 text-primary-700 dark:text-primary-400 border border-primary-200 dark:border-primary-900/50">
                    ...
                    <td className="px-6 py-4 text-xs font-semibold text-slate-500 dark:text-slate-400">
                      {u.profile?.location || 'India'}
                    </td>
                    <td className="px-6 py-4">
                      {u.isVerified ? (
                        <span className="flex items-center space-x-1 w-fit text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 border border-emerald-250 dark:border-emerald-900/50">
                          <Check className="w-3 h-3" />
                          <span>Verified</span>
                        </span>
                      ) : (
                        <span className="flex items-center space-x-1 w-fit text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-400 border border-slate-200 dark:border-slate-700">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end items-center space-x-2">
                        <button
                          ...
                          className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-800 text-slate-600 hover:text-primary-650 dark:text-slate-400 dark:hover:text-primary-400 hover:bg-slate-100 dark:hover:bg-slate-850 cursor-pointer transition-colors"
                        >
        ...
        {pages > 1 && (
          <div className="px-6 py-4 bg-slate-50 dark:bg-slate-950 border-t border-slate-100 dark:border-slate-850 flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500">
              Showing Page {page} of {pages} ({total} Users total)
            </span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800 rounded-3xl w-full max-w-lg shadow-2xl z-10 p-6 flex flex-col max-h-[90vh]">
            ...
            <div className="flex justify-between items-center mb-5 border-b border-slate-100 dark:border-slate-800 pb-4">
              <div>
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block font-outfit">User Profile Editor</span>
                <h3 className="font-outfit text-base font-extrabold text-slate-900 dark:text-white mt-0.5">
                  Update Settings for {editingUser.name}
                </h3>
              </div>
              <button onClick={() => setEditingUser(null)} className="p-1 rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-850 cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>

            <form onSubmit={handleEditSave} className="space-y-4 overflow-y-auto pr-1 flex-1">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-350 mb-1">Full Name</label>
                  <input
                    type="text"
                    required
                    value={editName}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-350 mb-1">Email Address</label>
                  <input
                    type="email"
                    required
                    value={editEmail}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-350 mb-1">System Role</label>
                  <select
                    value={editRole}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-350 mb-1">Verification Status</label>
                  <select
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-350 mb-1">Location</label>
                  <input
                    type="text"
                    value={editLocation}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-350 mb-1">Phone Number</label>
                  <input
                    type="text"
                    value={editPhone}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-350 mb-1">Bio</label>
                <textarea
                  value={editBio}
                  onChange={(e) => setEditBio(e.target.value)}
                  placeholder="Tell us about the user's business experience..."
                  rows="3"
